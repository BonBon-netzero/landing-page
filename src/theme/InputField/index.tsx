...
import Input, { InputNumber, InputPassword, Textarea } from 'theme/Input'
  ...
  (
    ...
    return (
      <Flex flexDirection="column" width={block ? '100%' : 'fit-content'} sx={sx}>
        ...
        />
        {!!error && (
          <Type.Caption color="danger2" mt={1} display="block">
            {error}
          </Type.Caption>
        )}
      </Flex>
...
export const InputNumberField = ({
  ...
  wrapperSx,
  ...props
}: { label?: ReactNode; wrapperSx?: any; required?: boolean } & ComponentProps<typeof InputNumber>) => {
  return (
    <Flex flexDirection="column" width={props.block ? '100%' : 'fit-content'} sx={wrapperSx}>
      {label && <Label label={label} error={props.error} required={required} />}
      <InputNumber {...props} />
      {!!props.error && (
        <Type.Caption color="danger2" mt={1} display="block">
          {props.error}

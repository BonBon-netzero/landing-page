...
import NoDataFound from '../NoContent/NoDataFound'
...
export type ColumnProps<T, K = unknown> = {
  ...
  align?: AlignType
  columnSx?: any
  render?: (text: T, index: number, externalSource?: K) => ReactNode
}
  ...
  return (
    <Box sx={{}}>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Flex minWidth={minWidth} justifyContent="space-between" color="neutral3" pb={2} px={3}>
          {columns &&
            columns.map((column) => (
              <Box key={column.key} textAlign={column.align} sx={column.columnSx}>
                <Type.Caption color="neutral4">{column.title}</Type.Caption>
              </Box>
            ))}
        </Flex>
            ...
            dataSource.map((data: any, index: any) => (
              <Box key={index} sx={{ px: 3, py: 2 }}>
                <Flex
                  ...
                  alignItems="center"
                  pb={2}
                  sx={{ borderBottom: '1px solid', borderColor: 'stroke' }}
                >
                  {columns &&
                    columns.map((column) => (
                      <Box key={column.key} textAlign={column.align} sx={column.columnSx}>
